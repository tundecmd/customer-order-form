import { customerOrderConstants, partsStockConfirmationConstants } from "./constants";
...
export const getPartsStockConfirmationById = (orderFormId) => {
  ...
  return async dispatch => {
    // console.log('a1')
    dispatch({ type: partsStockConfirmationConstants.GET_PARTS_STOCK_CONFIRMATION_BY_ID_REQUEST });
    // console.log("12345")
    const res = await axiosInstance.get(`/parts-stock-confirmation/${orderFormId}`);
    // console.log("56789")
...
export const getPartsStockConfirmation = () => {
  return async dispatch => {
    dispatch({ type: partsStockConfirmationConstants.GET_PARTS_STOCK_CONFIRMATION_LIST_REQUEST });
...
export const updateCustomerOrder = (orderFormId, payload) => {
  ...
  return async dispatch => {
    ...
    try {
      const res = await axiosInstance.put(`/customer-order-form/${orderFormId}`, payload);
