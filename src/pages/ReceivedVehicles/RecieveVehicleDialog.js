...
import { getCustomerOrder, updateCustomerOrder } from "state/actions";
import { getPartsStockConfirmation } from "state/actions/partsStockConfirmation.action";
...
export default function BasicTabs(props) {
    ...
    const {
        customerOrder,
        openOtherDetails1,
        setOpenOtherDetails1,
        receiveVehicleSubmitParentCallBack,
    } = props;
    ...
    const [Reception_Date, setReception_Date] = React.useState(
        customerOrder.Reception_Date
    );
    const [open, setOpen] = React.useState(false);
    const [Estimated_Job_Time, setEstimated_Job_Time] = React.useState(
        customerOrder.Estimated_Job_Time
    );
    const [Subtotal, setSubtotal] = React.useState(customerOrder.Subtotal);
    const [VAT_Amount, setVAT_Amount] = React.useState(
        customerOrder.VAT_Amount
    );
    const [Grand_Cost, setGrand_Cost] = React.useState(
        customerOrder.Grand_Cost
    );
    const [Customer_Signature, setCustomer_Signature] = React.useState(
        customerOrder.Estimated_Job_Time
    );

    const [Seat_Cover, setSeat_Cover] = React.useState(
        customerOrder.Seat_Cover
    );
    const [Floor_Mat, setFloor_Mat] = React.useState(customerOrder.Floor_Mat);
    const [Additional_Job_Confirmation, setAdditional_Job_Confirmation] =
        React.useState(customerOrder.Additional_Job_Confirmation);
    const [Replace_Parts_Keep, setReplace_Parts_Keep] = React.useState(
        customerOrder.Replace_Parts_Keep
    );
    const [Valuables, setValuables] = React.useState(customerOrder.Valuables);
    const [Present_Estimate_w_Explanation, setPresent_Estimate_w_Explanation] =
        React.useState(customerOrder.Present_Estimate_w_Explanation);
    const [Odometer_At_Appointment, setOdometer_At_Appointment] =
        React.useState(customerOrder.Odometer_At_Appointment);
    ...
    const captureRear = React.useCallback(
        (e) => {
            e.preventDefault();
            const imageSrc = webcamRefRear.current.getScreenshot({
                width: 1920,
                height: 1080,
            });
    ...
    const captureRight = React.useCallback(
        (e) => {
            e.preventDefault();
            const imageSrc = webcamRefRight.current.getScreenshot({
                width: 1920,
                height: 1080,
            });
    ...
    const captureLeft = React.useCallback(
        (e) => {
            e.preventDefault();
            const imageSrc = webcamRefLeft.current.getScreenshot({
                width: 1920,
                height: 1080,
            });
    ...
    const captureTop = React.useCallback(
        (e) => {
            e.preventDefault();
            const imageSrc = webcamRefTop.current.getScreenshot({
                width: 1920,
                height: 1080,
            });
    ...
    const [partsStockList, setpartsStockList] = React.useState(
        state.partsStock.partsStockList
    );
    ...
    const columnsPartsStockConfirmation = [
        ...
        },
        {
            ...
            render: (data) => {
                // console.log(data);
                return new Date("2019-02-19T06:00:00Z").toLocaleTimeString(
                    "en-gb"
                );
            },
        },
                ...
                <TabPanel value={value} index={0}>
                    ...
                    <Grid container spacing={1}>
                        ...
                        {/* Right Dialog starts here */}
                        {
                            <Dialog
                                ...
                                onClose={() =>
                                    setOpenModalRightImg(!openModalRightImg)
                                }
                            ...
                            >
                                <DialogContent>
                                    ...
                                    <span
                                        onClick={() =>
                                            setModalImageRight(!modalImageRight)
                                        }
                                    >
                                        {imgSrcRight && (
                                            <Box
                        ...
                        {/* left Dialog starts here */}
                        {
                            <Dialog
                                ...
                                onClose={() =>
                                    setOpenModalLeftImg(!openModalLeftImg)
                                }
                                aria-labelledby="responsive-dialog-title"
                            >
                                <DialogContent>
                                    ...
                                    <span
                                        onClick={() =>
                                            setModalImageLeft(!modalImageLeft)
                                        }
                                    >
                                        {imgSrcLeft && (
                                            <Box
                        ...
                        {/* front Dialog starts here */}
                        {
                            ...
                            >
                                <DialogContent>
                                    ...
                                    <span
                                        onClick={() =>
                                            setModalImageFront(!modalImageFront)
                                        }
                                    >
                                        {imgSrcFront && (
                                            <Box
                        ...
                        {/* rear Dialog starts here */}
                        {
                            <Dialog
                                ...
                                onClose={() =>
                                    setOpenModalRearImg(!openModalRearImg)
                                }
                                aria-labelledby="responsive-dialog-title"
                            >
                                <DialogContent>
                                    ...
                                    <span
                                        onClick={() =>
                                            setModalImageRear(!modalImageRear)
                                        }
                                    >
                                        {imgSrcRear && (
                                            <Box
                        ...
                        {/* top Dialog starts here */}
                        {
                            <Dialog
                                ...
                                onClose={() =>
                                    setOpenModalTopImg(!openModalTopImg)
                                }
                                aria-labelledby="responsive-dialog-title"
                            >
                                <DialogContent>
                                    ...
                                    <span
                                        onClick={() =>
                                            setModalImageTop(!modalImageTop)
                                        }
                                    >
                                        {imgSrcTop && (
                                            <Box
                    ...
                    <form className="form" onSubmit={handleSubmit}>
                        ...
                        <FormControlLabel
                            ...
                            label={
                                <Typography variant="body1">
                                    Present Estimate With Explanation
                                </Typography>
                            }
                        />
                        <FormControlLabel
                        ...
                        <div className="formGroup">
                            ...
                            <input
                                type="text"
                                name="Odometer_At_Appointment"
                                value={Odometer_At_Appointment}
                                onChange={(e) =>
                                    setOdometer_At_Appointment(e.target.value)
                                }
                                className="formField"
                            />
                        </div>
        ...
        >
          Continue
                ...
                <TabPanel value={value} index={1}>
                    ...
                    <div className="formGroup">
                        <label htmlFor="label" className="formLabel">
                            I have received an explanation of and agreed to the
                            items listed above and accepted these terms and
                            conditions.{" "}
                        </label>
                        <textarea
                            rows={4}
