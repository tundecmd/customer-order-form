...
function CustomDataTable(props) {
  ...
  const [isExpectedVehicleDialogOpen, setIsExpectedVehicleDialogOpen] = useState(false);
  const [selectExpectedVehicleDialogRow, setSelectExpectedVehicleDialogRow] = useState(null);
  ...
  const checkIn = () => {
    ...
    if (dispatch(updateCustomerOrder(selectedCustomerOrder.No, formObject))) {
  ...
  const handleSelectCustomerOrder = (selectedCustomerOrder) => {
    setSelectedCustomerOrder(selectedCustomerOrder);
    console.log('selectedCustomerOrder', selectedCustomerOrder);
    history.push(`job-instruction/${selectedCustomerOrder.No}`);
